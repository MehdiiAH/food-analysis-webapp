...
def extract_text_from_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Args : df, a dataframe containing the recipes to be searched

    Return : a dataframe with merged colums for name and description
...
def extract_tokens_from_df(df: pd.DataFrame) -> Tuple[Iterable[Doc], Set[str]]:
    ...
    Args : df, a dataframe containing the recipes concatenated

    Return : a list of docs generated by the NLP pipeline and a list of stopwords
...
def extract_tokens_from_doc(doc: Doc, stopwords: Set[str]) -> List[str]:
    ...
    Args: doc, a doc extracted with the NLP pipe
            stopwords, the list of stopwords from the same pipe

    Return: a list of interesting tokens
...
def store_tokens_in_df(
    ...
    Args: docs, a list of doc extracted from the recipe dataframe
            stopwords, the list of stopwords from the NLP pipe
            df, the recipe dataframe in which storing the results

    Return: the recipe dataframe updated with the tokens
    """
